...
export function ContactApp() {
  ...
  return (
    <div className="font-mono space-y-6 sm:space-y-8 w-full pb-6 text-zinc-100 select-text">
      ...
      <div className="space-y-4 pt-2">
        {links.map((link) => {
          ...
          return (
            <div
              key={link.name}
              className="p-4 sm:p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800/80 hover:border-brand/50 transition-all flex items-center justify-between gap-4 group shadow-lg"
            >
              <div className="flex items-center space-x-3.5 min-w-0">
                <div className="p-3 rounded-xl bg-zinc-800 text-brand group-hover:bg-brand/15 transition-colors shrink-0">
                  <Icon className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <p className="text-xs text-zinc-400 font-mono">{link.name}</p>
                  <p className="text-sm sm:text-base font-semibold text-zinc-100 font-mono mt-0.5">
                    {link.handle}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2 shrink-0">
                <button
                  ...
                  className="px-3 py-2 rounded-lg text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-300 transition-colors flex items-center space-x-1.5 font-mono"
                  title="Copy URL"
                >
                  {isCopied ? (
                    <>
